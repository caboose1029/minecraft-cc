...
- **Create 6.0.10** — the core automation mod this whole project targets. Primary source for job-types/resource-tree entries: milling (ore→crushed ore), smelting (crushed ore→ingot, vanilla furnace), pressing (ingot→sheet), Andesite Alloy, Brass Ingot, and Andesite/Brass/Copper Casing (Deployer-applied, not crafting-table) are all in `config/resource-tree.json` now. Two verified-but-excluded findings worth remembering: Create's primary wikis (create.fandom.com, createmod.wiki, wiki.createmod.net) returned 402/403/404 to WebFetch during research — data came from a GitHub-wiki mirror plus WebSearch synthesis instead, and should still be spot-checked against JEI; and **Precision Mechanism uses Sequenced Assembly** (an ordered multi-step Deployer sequence with a final success-*chance*), a genuinely different recipe shape the current schema can't express — deliberately left unmodeled rather than forced in, see `PLAN.md`.
