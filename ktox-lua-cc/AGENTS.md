...
- `resource-tree.json`'s vanilla Minecraft coverage is still thin. Mod-jar extraction (see "Modpack" above) is the authoritative source for Create/addon recipes, but vanilla's own recipes aren't shipped inside any mod jar, so they've only been added piecemeal from general knowledge so far (basic smelting/smoking, chest, dried kelp block). Vanilla recipes are simple and stable enough that general knowledge is reliable here, but "exhaustive" hasn't been attempted — expand this deliberately (e.g. a pass through vanilla's own data pack, which ships in the game's own jar the same way mod recipes ship in mod jars) rather than continuing to add entries reactively one at a time.
