...
- **Create 6.0.10** — the core automation mod this whole project targets. Primary source for job-types/resource-tree entries: milling (ore→crushed ore), smelting (crushed ore→ingot, vanilla furnace), pressing (ingot→sheet), Andesite Alloy, Brass Ingot, Andesite/Brass/Copper Casing (Deployer-applied, not crafting-table), and Washing/Splashing's deterministic recipes (Ice, Wheat Flour, Magma Block) are all in `config/resource-tree.json` now. Create's primary wikis (create.fandom.com, createmod.wiki, wiki.createmod.net) returned 402/403/404 to WebFetch across both research passes — data came from a GitHub-wiki mirror plus WebSearch synthesis instead, and should still be spot-checked against JEI. Two mechanics deliberately left unmodeled, both discussed at length in `PLAN.md`: **Precision Mechanism** uses Sequenced Assembly (an ordered multi-step Deployer sequence with a final success-*chance*), a genuinely different recipe shape the schema can't express; and **Splashing** ("washing" — an Encased Fan blowing through a water source block, *not* a Water Wheel as first assumed here) is mostly chance-based with zero deterministic yield (e.g. Gravel → Flint 25% or Iron Nugget 12.5%, nothing guaranteed), which is also why the well-known "endless cobblestone → iron/andesite" automation chain is treated as an external farm dumping into storage, not a job — it's built entirely on chance-based steps plus a fluid-consuming Andesite recipe, doesn't fit the job schema on two separate counts, and is "endless" (always-running) by design anyway.
