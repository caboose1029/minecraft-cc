...
**Confidence levels matter here** — base Create and vanilla Minecraft recipes are well-established, high-confidence knowledge, backed by an actual research pass (see above); the smaller addons are not, and several were excluded entirely rather than guessed at. Treat `resource-tree.json` as a solid core plus a starting point — verify against JEI (installed) before trusting the Farmer's Delight-derived entries for anything real. Update this section (and re-derive job-types/resource-tree) if the mod list changes.
