...
Until that's fixed, `GhFetch.lua` is the only way scripts actually reach a fresh turtle/computer. It's hand-written Lua, not ktox-generated (a deliberate exception — see below): calling through `require("ktox-lib")`/the shim like every ktox program does would mean GhFetch itself needs `ktox-lib.lua`/`ktox-cc-shim.lua` already present and dofile'd by `startup.lua` (i.e. a reboot) before it could even run, turning bootstrap into "wget 4 files, reboot, then run GhFetch." Pure Lua talking to `http`/`fs` directly collapses that to "wget this one file, then run it" — the reboot still happens, but only once, after GhFetch has already fetched everything (including its own ktox-lib/shim/startup dependencies via the manifest), not as a prerequisite to running it at all. It fetches `files.manifest` (source: `src/main/lua/files.manifest`, hand-written like `startup.lua`/`ktox-cc-shim.lua`, copied into the output tree by the same `copyLuaRuntime` Gradle task) and downloads whatever that lists, one path per line. It's still a hand-maintained list — nothing derives it from the actual file tree — but editing it needs only a plain-text edit and a commit, same as before (GhFetch itself never needed a Kotlin rebuild for this specifically — see PLAN.md's "GhFetch's file manifest" for that history). **Every new program or `lib/` file must be proactively added to `files.manifest`** as part of the same change, or it will silently be missing from `ghfetch` runs. Drop this requirement once moonman's `sync` is fixed and this project can rely on his manifest/sync instead.

`copyLuaRuntime` (the Gradle task that copies `src/main/lua/*` into the output tree, GhFetch.lua included) must run **after** `transpileKotlinToLua`, not just be left unordered relative to it — ktox's transpile step deletes output files that no longer correspond to a Kotlin source, which silently deleted a freshly-copied `GhFetch.lua` the moment `GhFetch.kt` was removed, until `copyLuaRuntime` was given an explicit `dependsOn("transpileKotlinToLua")` in `build.gradle.kts`. Watch for this again if another hand-written file is ever added to replace a Kotlin one.
...
- `src/main/lua/` — hand-written Lua that ships alongside the transpiled output: `ktox-cc-shim.lua` (multi-return native-call wrappers), `startup.lua` (loads the shim at boot), and `GhFetch.lua` (deliberately pure/dependency-free — see above — the bootstrap entry point, not a ktox program like everything else in `programs/`). Copied into the output tree by the `copyLuaRuntime` Gradle task, not ktox-generated.
...
The vault terminal's `config/job-types.lua`/`config/resource-tree.lua` (see `ktox-lua-cc/PLAN.md`) are built against this specific mod list — **Minecraft 1.21.1, NeoForge**. Source of truth for what's installed: `ls -a ~/Downloads/Mods8.16.2026` on the user's machine (that directory name is just where the jars happen to sit, not itself meaningful). Snapshot as of 2026-09-07:

- **Create 6.0.10** — the core automation mod this whole project targets. **Source of truth changed mid-project: extract real recipe JSON straight from the mod `.jar` (a ZIP archive containing `data/create/recipe/**/*.json` for every recipe, ~1884 files) instead of researching from memory/WebSearch.** Web research had a real, confirmed error rate — wrong item IDs (`create:crushed_iron_ore` doesn't exist; it's `create:crushed_raw_iron`), a wrong mechanic assumption (`create:milling` and `create:crushing` were assumed to be the same recipe type at different speeds; they're genuinely different machines with non-overlapping recipe sets — Millstone mills plants/wheat, Crushing Wheels crush ores), wrong namespaces (`wheat_flour`/`dough` are `create:`, not `farmersdelight:`), and one fully fabricated recipe. `unzip -l <jar> | grep recipe`, then reading the JSON directly, is fast and authoritative — do this again before trusting any new Create entry, not another research pass. See `PLAN.md`'s "Content build, v2" for the full list of what got corrected and the current per-entry confidence levels. Two mechanics still deliberately left unmodeled: **Precision Mechanism** uses Sequenced Assembly (an ordered multi-step Deployer sequence with a final success-*chance*), a genuinely different recipe shape the schema can't express; and **Splashing** ("washing" — an Encased Fan blowing through a water source block, *not* a Water Wheel as first assumed) is mostly chance-based with zero deterministic yield (e.g. Gravel → Flint 25% or Iron Nugget 12.5%, nothing guaranteed, confirmed via the jar's own `"chance"` fields), which is also why the well-known "endless cobblestone → iron/andesite" chain is treated as an external farm, not a job.
- **Create Aeronautics (bundled) 1.3.0** — confirmed via CurseForge/official docs: vehicle/contraption building (airships, planes, drones), using Create's kinetics + the Sable physics library (that's why Sable is in this list — a dependency, not standalone content). No evidence of new material-processing recipes; excluded from resource-tree.lua.
- **Create: Enchantment Industry 2.4.2** — confirmed: automates enchanting (Mechanical Grindstone, Blaze Enchanter/Forger, Experience Hatch). Core mechanism runs on **Liquid Experience** (a fluid) — excluded entirely, this system has no fluid support (see `PLAN.md`).
- **Create Dragons Plus 1.11.7b** — confirmed, and not what the name suggests: a dev-utility/QoL addon (bulk dye/freeze/sand/end effects via an Encased Fan), not dragon-mob content. Its "apply an effect via fan to a passing item" mechanic doesn't map to a discrete input→output recipe — excluded.
- **Create Food (createfood) 2.7.1** — confirmed: an addon for Farmer's Delight adding 200+ food items/100+ fluids via tag-based recipes. Too broad to enumerate specific recipes with confidence from available sources — excluded, not fabricated.
- **Slice & Dice 4.3.3** — confirmed, and the actually load-bearing addon here: its Slicer machine automates every Farmer's Delight Cutting Board recipe (knife/axe placed in the machine, item passes through), and its Heated Mixing automates every Farmer's Delight Cooking Pot recipe (Basin + Mixer + heat). This is the bridge that makes Farmer's Delight items automatable at all, and is why `resource-tree.lua` has a `"slicer"` and `"cooking_pot"` job type.
- **Farmer's Delight 1.3.2** — confirmed via the mod's own wiki. Cutting Board (125 recipes total) and Cooking Pot (27 recipes) are the two stations referenced above; only a handful of representative recipes (log stripping, chicken cuts, pumpkin slice, beef stew) are in `resource-tree.lua` so far — exact item IDs for the Farmer's Delight-namespaced items are plausible but not individually re-verified, lower confidence than the core Create entries.
- **Bits n Bobs 0.0.44** — confirmed to exist (3M+ downloads) but no specific recipe data found; unknown whether it adds processing-relevant recipes. Not included.
- **Climbable Ropes 2.1.1** — could not confirm as a distinct mod at all (search only surfaced Farmer's Delight's own built-in rope item). Not included.
- **Sable 2.0.3** — confirmed as a pure physics-engine library (used by Create Aeronautics) with zero survival content. Not a recipe source at all, excluded outright.
- **Display Delight 1.7.0** — could not find this mod under this name via any search. Completely unverified; nothing included for it rather than fabricate.
...
**Confidence levels matter here** — base Create and vanilla Minecraft recipes are well-established, high-confidence knowledge, backed by an actual research pass (see above); the smaller addons are not, and several were excluded entirely rather than guessed at. Treat `resource-tree.lua` as a solid core plus a starting point — verify against JEI (installed) before trusting the Farmer's Delight-derived entries for anything real. Update this section (and re-derive job-types/resource-tree) if the mod list changes.
...
- **A top-level `const val` in a `common/` file is a real Lua global assignment — but it never runs, because `common/*.lua` files are never `dofile`'d.** The enum-class quirk above says "use `const val` instead, those transpire to genuine global assignments" — true, but that's only useful if the containing *file* actually executes at some point (entry point, `dofile`, or `require`). `lib/*.lua` files get `dofile`'d by `startup.lua` automatically; `common/*.lua` files are deliberately NOT on that list, because by convention they only ever contain `@NativeName`/`externalSource()` declarations, which generate no runtime code at all — so historically it never mattered that they're skipped. A `const val` placed in a `common/` file breaks that assumption silently: the assignment statement exists in the generated `.lua`, but since the file is never executed, the global stays `nil` forever. Confirmed live: `common/Peripheral.kt` defined `const val KTOX_PERIPHERAL_MISSING = "MISSING"`, used from `lib/Redstone.kt` as `result != KTOX_PERIPHERAL_MISSING` — since the real global was `nil` (not the string), the comparison silently became `result != nil`, and a peripheral that was genuinely missing (whose call correctly returned the *string* `"MISSING"`) compared as `~= nil` → `true`, reporting success for a call that never reached anything. No error anywhere — it just quietly returned the wrong boolean. Fix: never define a real (non-native-binding) constant in a `common/` file; put it in a `lib/` file instead (dofile'd, safe), or simplest — skip the shared constant entirely and compare against the literal string at each call site, matching what the Lua shim itself does.
...
- `resource-tree.lua`'s mod coverage is now comprehensive (a scripted full-corpus extraction across every installed mod's jar — see PLAN.md "Comprehensive extraction pass, v3"), but vanilla Minecraft's own recipes (ore smelting, food smoking, vanilla crafting-table tools/blocks) are still only a small hand-curated general-knowledge block, since vanilla's own recipes aren't shipped inside any MOD jar — only the base game's own client/server jar has them, which this project has no copy of. If a vanilla client/server jar (or a MultiMC/Prism instance directory containing one) ever becomes available, re-run the same extraction script against `data/minecraft/recipe/**/*.json` from it for the same jar-verified confidence level the mod content now has.
